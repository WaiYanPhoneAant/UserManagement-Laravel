...
@section('header','Role Edit Form')
@section('content')

<div class="m-3 mx-2 pb-5">
    <h1 class="fw-light m-2 mb-5 d-md-none d-block">Role Edit Form</h1>
    <div class="form mt-3">
            ...
            @csrf
                <div class="name-input row m-0">
                    <div class="mb-3 col-md-3">
                        ...
                        <input type="text" class="form-control" id="name" name="roleName" value="{{old('roleName',$role->name)}}" aria-describedby="emailHelp" placeholder="Enter Role Name">
                        @error('roleName')
                                        ...
                                        <div class="ms-3">
                                            @foreach ($permissions[$f->name] as $p)
                                                <div class="form-check mb-2">
                                                    ...
                                                    @foreach ($permissions_id as $c)
                                                        {{$c->permissions_id==$p->permissions_id?'checked':''}}
                                                    @endforeach

                                                    type="checkbox" name=" {{$p->permission.$p->feature}}" value=" {{$p->permissions_id}}" id="{{$p->permission.$p->feature}}" >
                ...
                <div class="d-flex justify-content-center mt-3">
                    <button type="submit" class="mt-5 align-items-center btn btn-lg btn-primary">Save Change</button>
                </div>
        </form>
    </div>
</div>
